...
1. A probabilidade de $(X,Y)$ pertencer a uma região de $\mathbb{R}^2$ é a integral dupla de $f(x,y)$ nessa região. Por exemplo, $P(a < X \leq b, c < X \leq d) = \int_a^b\int_c^d f(x,y) dy dx$;
...
```{r out.width="75%", fig.height=5}
#| layout-ncol: 2

x <- seq(0,1, length.out=1000)
fx <- 12*x*(1-x)^2

par(family = "serif", mar=c(3.5,3.5,.3,.3))
...
Seja $(X,Y)$ vetor aleatório discreto com fmpc $p(x,y)$, então a fmp condicional de $Y$ dado que $X = x$ é dada por
<table style="margin-left: auto; margin-right: auto;">
  <tr style="border-bottom:none">
    <td>$p_{Y|X}(y|x) = \frac{p(x,y)}{p_X(x)}$,</td>
  </tr>
</table>
isto é, a fmpc de $(X,Y)$ dividida pela fmp marginal de $X$.
:::

::: {.callout-note}
## Definição

Seja $(X,Y)$ vetor aleatório contínuo com fdpc $f(x,y)$, então a fdp condicional de $Y$ dado que $X = x$ é dada por
<table style="margin-left: auto; margin-right: auto;">
  <tr style="border-bottom:none">
    <td>$f_{Y|X}(y|x) = \frac{f(x,y)}{f_X(x)}$,</td>
  </tr>
</table>
isto é, a fdpc de $(X,Y)$ dividida pela fdp marginal de $X$.
:::
