...
**Exemplo 15:** Suponha que 25% dos motoristas habilitados em um estado não tenham seguro. Seja $X =$ "número de motoristas sem seguro em uma amostra com reposição de tamanho 100". Se representarmos $s$ o motorista não ter seguro, então $X \sim \text{Bin}(100, 0.25)$. Para determinar $P(X \leq 20)$ teríamos que fazer
$$\textstyle P(X \leq 20) = \sum_{x=0}^{20} p_X(x) = p_X(0) + p_X(1) + \cdots p_X(20) = \sum_{x=0}^{20} {100 \choose x} 0.25^x0.75^{100-x} \approx 0.1488.$$
