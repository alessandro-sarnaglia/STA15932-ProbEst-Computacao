...
Embora, em geral, as probabilidades da distribuição Gama não poderem ser obtidas analiticamente, o caso especial em que $\alpha = 1$ permite a sua obtenção. Esse caso particular da distribuição gama é chamada de distribuição [**exponencial**]{style='color:red;'}.
:::
...
Dizemos que $X$ tem distribuição exponencial com taxa $\lambda$, denotada por $X \sim \text{Exp}(\lambda)$, se sua fdp é dada por
<table>
  <tr style="border-bottom:none;text-align:center;">
...
[Seja $\tilde{\mu}$ o tempo de resposta mediano. Como $F_X(\tilde{\mu}) = 1-e^{-0.2\cdot \tilde{\mu}} = 0.5$,]{.fragment} [temos que a mediana é $\tilde{\mu} = -5\log(0.5) \approx 8.0472$.]{.fragment} [Note que $\tilde{\mu} \approx 8.0472 > 5 = \mu$, o que reflete a assimetria à direita da distribuição exponencial.]{.fragment}
