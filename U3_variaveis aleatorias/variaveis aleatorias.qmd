...
1. No Exemplo 12, temos que $X \sim \text{BN}(k=3, p)$. Se $p = 0.01$, por exemplo, teríamos $\mu = E(X) = \frac{3}{0.01} = 300$, $\sigma^2 = V(X) = 3\frac{0.99}{0.01^2} = 29700$ e $\sigma \approx 172.34$. Isto é, em média, seriam realizadas $300$ transações até a terceira falha com DP de $172.34$;
...
Anteriormente, definimos uma vac $X$ pelas características de seu conjunto imagem $\text{Im}(X)$. Abaixo, iremos nos restringir àquelas em que a fda é absolutamente contínua.
:::

::: {.callout-note}
## Definição

Seja $X$ vac com fda $F_X(x) = P(X \leq x)$ absolutamente contínua. Então,
