...
$$
Pode ser interesse registrar apenas o número de componentes defeituosos e não as falhas individuais, principalmente se $n$ fosse grande. Nesse caso, teríamos $X(\omega) =$ "número de componentes defeituosos em $\omega$". Assim, teríamos
...
1.    A qualquer va $X$, podemos associar um contradomínio (normalmente o conjunto $\mathbb{R}$) e uma **imagem**, que denotaremos por $\text{Im}(X)$;
2.    No exemplo anterior, temos que $\text{Im}(X) = \{0,1,2,3\}$.
...
O conjunto de todos possíveis valores que uma va $X$ pode assumir é denominado [**imagem**]{style="color:red;"} de $X$ e é denotado por $\text{Im}(X)$.
...
Um evento aleatório muito importante para modelagem probabilística de va's é $[X \leq x]$, isto é a va $X$ assumir valor menor ou igual a $x$.
...
No caso discreto, a função $p_X(x) = P(X=x)$, $x \in \text{Im}(X)$, é denominada [**função massa de probabilidade**]{style="color:red;"} (fmp).
:::

::: {.callout-caution}
## Observação

Como consequência, vemos que, para variáveis aleatórias discretas, a fda $F_X(x)$ nos fornece a fmp $p_X(x)$ e vice-versa.
:::
...
Se $X$ é vac com fda $F_X(x)$, então a sua **função densidade de probabilidade** (fdp) é definida por
$$\textstyle f_X(x) = \frac{dF_X(x)}{dx}.$$
:::
