...
**Exemplo 6 (cont.):** A função de verossimilhança se a população $X\sim \text{Exp}(\lambda)$ é
<table>
  <tr style="border-bottom:none;text-align:center;">
    <td>$L(\lambda; x_1, \ldots, x_n) = \prod_{i=1}^n[\lambda e^{-\lambda x_i}] = \lambda^n e^{-\lambda \sum_ix_i} = \lambda^n e^{-\lambda n\bar{x}}$.</td>
  </tr>
</table>
Assim, $\ell(\lambda) = n\log(\lambda)-\lambda n \bar{x}$ e $\frac{\partial\ell(\lambda)}{\partial\lambda} = \frac{n}{\lambda} - n\bar{x}$. Como $\hat\lambda_{\text{MV}}$ satisfaz $\frac{n}{\hat\lambda_{\text{MV}}} - n\bar{x} = 0$, temos $\hat\lambda_{\text{MV}} = \frac{1}{\bar{X}}$.
...
o que fornece $\hat\mu_{\text{MV}} = \bar{X}$ e $\hat\sigma_{\text{MV}} = \left[\frac{1}{n}\sum_i(X_i - \hat\mu_{\text{MV}})^2\right]^{1/2}$.
:::
