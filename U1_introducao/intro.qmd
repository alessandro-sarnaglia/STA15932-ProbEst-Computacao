...
media <- 5*as.numeric(dia_util=='Sim') + 1*as.numeric(dia_util=='Não')
atendimentos <- rpois(n = length(media), lambda = media)
df <- data.frame("Dia Útil" = dia_util, "Atendimentos" = atendimentos)
df %>%
...
library(gt)
library(gtsummary)
df0 <- data.frame(Frequencia = 'Frequência', Atendimentos = df$Atendimentos)
...
## <span style="font-size:80%">Definição</span>
...
<span style="color: red;"> **Importante:** </span> Se a diferença for muito grande, a comparação nos subgrupos fica comprometida.
...
<span style="color: blue;"> **Observação:** </span> As frequências relativas podem ser apresentadas em percentual.

:::

:::

## <span style="font-size:80%">Exemplo anterior</span>
...
-   Isso significa <span style="color:red;"> distribuição dos atendimentos por dia útil</span>;
-   Caso necessário, poderíamos calcular os percentuais por coluna. Nesse caso, teríamos a <span style="color:red;"> distribuição do dia útil por atendimentos</span>.
:::
