...
::: incremental
-   tendência central da variável [⟹ [Exemplos: média, mediana e moda;]{style="color:red; "}]{.fragment}
-   dispersão da variável [⟹ [Exemplos: variância e desvio-padrão.]{style="color:red; "}]{.fragment}
:::
...
::: incremental
-   ponto de equilíbrio dos dados [⟹ [média (aritmética);]{style="color:red; "}]{.fragment}
-   ponto central dos dados [⟹ [mediana;]{style="color:red; "}]{.fragment}
-   ponto de alta frequência [⟹ [moda.]{style="color:red; "}]{.fragment}
:::
...
Dadas $n$ observações $x_1, \ldots, x_n$ de uma variável quantitativa, a sua média (aritmética) é definida por
$$\bar{x} = \frac{1}{n}\sum_{i=1}^n x_i.$$

:::: {.columns .v-center-container}

::: {.column width="65%"}
Se os dados estão na forma de distribuição de frequências não intervalar [[como a do lado]{.fragment .highlight-current-red fragment-index=1}]{.fragment fragment-index=1}[, podemos utilizar uma das seguintes expressões:]{.fragment  fragment-index=2}
...
::: fragment
**Definição:** Dada a $i$-ésima faixa $(a_{i-1}, a_i]$, o seu ponto médio é definido por $x_i^{(m)}$ e média é aproximada por
$$\bar{x} \approx \frac{1}{n}\sum_{i=1}^kx_i^{(m)}n_i = \sum_{i=1}^kx_i^{(m)}f_i.$$
:::
